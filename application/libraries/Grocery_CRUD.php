	...
	protected function delete_file($state_info)
	{
		...
		if(isset($state_info->field_name) && isset($this->upload_fields[$state_info->field_name]))
		{
			...
			if(file_exists("{$upload_info->upload_path}/{$state_info->file_name}"))
			{
				...
				{
                    $thumb_name = thumb_name(TERMEKLISTA_THUMB_WIDTH, $state_info->file_name);     //gergo
                    if(file_exists("{$upload_info->upload_path}/$thumb_name"))    //gergo                 
                    unlink("{$upload_info->upload_path}/$thumb_name");            //gergo
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_readonly_input($field_info, $value) //gergo
	{
        
        if (!empty($field_info->db_type) AND (
            $field_info->db_type == 'int' OR
            $field_info->db_type == 'tinyint'
        )){
                $read_only_value = 0;
        } else {;
                $read_only_value = "&nbsp;";
	...
	protected function get_upload_file_readonly_input($field_info,$value)
	{
		...
		$value = !empty($value) ? '<img style="max-width:200px;" src="'.$file.'"/>' : ''; //gergo
...
class UploadHandler
{
    ...
    private function trim_file_name($name, $type) {
        ...
        $file_name = $this->_transliterate_characters($file_name); //gergo
